...
const keywordSuggestions = require('./keywordSuggestions');

class KeywordServices {
  /**
   * Analyze keyword density with detailed metrics and visualization data
   * @param {string} content - HTML or text content
   * @param {Array<string>|string} keywords - Keywords to analyze (array or comma-separated)
   * @returns {Object} Detailed density analysis with visualization data
   */
  static analyzeKeywordDensity(content, keywords) {
    ...
    try {
      ...
      console.log(
        `[KEYWORD-SERVICES] Analyzing ${keywordList.length} keywords: ${keywordList.slice(0, 3).join(', ')}${keywordList.length > 3 ? '...' : ''}`
      );
      ...
      const densityResults = keywordList.map(keyword => {
        const result = this._calculateDensity(text, keyword, wordCount);
        return result;
      });
   ...
   * @private
   */
  static _calculateDensity(text, keyword, totalWords) {
   ...
   * @private
   */
  static _calculateDistribution(text, keywords) {
   ...
   * @private
   */
  static _generateDensityRecommendations(densityResults) {
    const recommendations = [];
   ...
   * @param {string} content - HTML or text content
   * @param {Array<string>} seedKeywords - Base keywords to expand
   * @param {number} maxSuggestions - Maximum suggestions to return
   * @returns {Object} Long-tail keyword suggestions
   */
  static generateLongTailKeywords(
    ...
    try {
      ...
      console.log(
        '[KEYWORD-SERVICES] Long-tail generation suggestions: ',
        result.suggestions
      );
      return result;
    } catch (error) {
   ...
   * @private
   */
  static _extractPhrases(text, minWords, maxWords) {
    const words = text
      .toLowerCase()
      .split(/\s+/)
   ...
   * @private
   */
  static _scoreLongTailPhrase(phraseObj, seedKeywords, _fullText) {
    // Validate input
    if (
      !phraseObj ||
      !('phrase' in phraseObj) ||
   ...
   * @private
   */
  static _categorizeByIntent(scoredPhrases) {
    const categories = {
      informational: [],
      commercial: [],
      navigational: [],
   ...
   * @param {string} content - Content context (optional)
   * @returns {Object} Difficulty estimates for each keyword
   */
  static estimateKeywordDifficulty(keywords, content = '') {
    const startTime = Date.now();
    ...
    try {
      ...
      const estimates = keywordList.map(keyword => {
        const difficulty = this._calculateDifficulty(keyword, content);
        return difficulty;
      });
   ...
   * @private
   */
  static _calculateDifficulty(keyword, _content) {
   ...
   * @private
   */
  static _getDifficultyRecommendation(level, keyword) {
    switch (level) {
      case 'easy':
   ...
   * @param {Array<string>} keywords - Keywords to cluster
   * @param {string} _content - Content for context (optional, reserved for future use)
   * @param {Object} options - Clustering options
   * @param {number} options.similarityThreshold - Similarity threshold (0-1, default: 0.3)
   * @param {string} options.strategy - Clustering strategy: 'jaccard' or 'semantic' (default: 'jaccard')
   * @returns {Object} Enhanced clustered keywords with analysis
   */
  static clusterKeywords(keywords, _content = '', options = {}) {
    const startTime = Date.now();
    ...
    try {
      ...
      const result = {
        ...
        options: { similarityThreshold, strategy },
   ...
   * @private
   */
  static _calculateSimilarity(keyword1, keyword2, strategy = 'jaccard') {
   ...
   * @private
   */
  static _calculateSemanticSimilarity(keyword1, keyword2) {
   ...
   * @private
   */
  static _wordSimilarity(word1, word2) {
   ...
   * @private
   */
  static _findCommonWords(keywords) {
   ...
   * @private
   */
  static _generateClusterName(keywords) {
   ...
   * @private
   */
  static _calculateClusterQuality(cluster) {
    let score = 0;
   ...
   * @private
   */
  static _identifyClusterTheme(keywords) {
    const themes = {
   ...
   * @private
   */
  static _generateClusteringInsights(clusters, totalKeywords) {
    const insights = [];
   ...
   * @param {Array<string>} mainKeywords - Main keywords
   * @param {number} maxSuggestions - Maximum suggestions
   * @returns {Object} LSI keyword suggestions
   */
  static generateLSIKeywords(content, mainKeywords = [], maxSuggestions = 15) {
    const startTime = Date.now();
    ...
    try {
      ...
      const lsiCandidates = suggestions.filter(
        s => !mainKeywordsLower.includes(s.keyword.toLowerCase())
      );
   ...
   * @private
   */
  static _calculateLSIScore(candidate, mainKeywords, text) {
...
module.exports = KeywordServices;
