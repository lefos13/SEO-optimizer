...
const rules = [
...

  {
    ...
    check: async content => {
      ...
      const message = hasSchema
        ? `Schema markup detected: ${[hasJsonLd && 'JSON-LD', hasMicrodata && 'Microdata', hasRDFa && 'RDFa'].filter(Boolean).join(', ')}`
  ...
  // ============================================================
  {
    ...
    check: async content => {
      ...
      const passed = viewport && viewport.length > 0;
      const message = passed
...

  {
    ...
    check: async content => {
      ...
      const passed = canonical && canonical.length > 0;
      const message = passed
...

  {
    ...
    check: async content => {
      ...
      const passed = language && language.length > 0;
      const message = passed
...

  {
    ...
    check: async content => {
      ...
      const passed = charset && charset.length > 0;
...
/**
 * Get all rules
 * @returns {Array<Object>} All SEO rules
 */
 ...
 * @returns {Array<Object>} Rules in category
 */
 ...
 * @returns {Array<Object>} Rules with given severity
 */
