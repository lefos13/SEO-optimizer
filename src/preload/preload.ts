...
export interface ElectronAPI {
  ...
  seo: {
    ...
    updateRecommendationStatus: (recId: number, status: string, notes?: string) => Promise<void>;
    getQuickWins: (analysisId: number) => Promise<unknown[]>;
    calculateDensity: (text: string, keyword: string) => Promise<number>;
    calculateDensities: (text: string, keywords: string[]) => Promise<Record<string, number>>;
    suggestKeywords: (html: string, maxSuggestions?: number) => Promise<unknown[]>;
    saveRecommendations: (analysisId: number, enhancedRecommendations: unknown[]) => Promise<void>;
    fetchUrl: (url: string, options?: unknown) => Promise<unknown>;
  };
  keyword: {
    analyzeDensity: (content: string, keywords: string[]) => Promise<unknown>;
    generateLongTail: (content: string, seedKeywords: string[], maxSuggestions?: number) => Promise<unknown[]>;
    estimateDifficulty: (keywords: string[], content: string) => Promise<unknown>;
    cluster: (keywords: string[], content: string) => Promise<unknown>;
    generateLSI: (content: string, mainKeywords: string[], maxSuggestions?: number) => Promise<unknown[]>;
  };
  readability: {
    analyze: (content: string, options?: { language?: string }) => Promise<unknown>;
    analyzeOverview: (content: string, options?: { language?: string }) => Promise<unknown>;
    analyzeStructure: (content: string, options?: { language?: string }) => Promise<unknown>;
    analyzeReadingLevels: (content: string, options?: { language?: string }) => Promise<unknown>;
    analyzeImprovements: (content: string, options?: { language?: string }) => Promise<unknown>;
    analyzeLanguageGuidance: (content: string, options?: { language?: string }) => Promise<unknown>;
    analyzeLiveScore: (content: string, options?: { language?: string }) => Promise<unknown>;
  };
  content: {
    ...
    recommendInternalLinks: (content: string, existingPages: unknown[]) => Promise<unknown>;
    optimizeLength: (content: string, options?: unknown) => Promise<unknown>;
    analyzeGaps: (content: string, topics: string[]) => Promise<unknown>;
    analyzeCompetitive: (content: string, competitors: unknown[]) => Promise<unknown>;
  };
}
...
contextBridge.exposeInMainWorld('electronAPI', {
  // Projects
  projects: {
    create: (projectData: unknown) => ipcRenderer.invoke('db:project:create', projectData),
    get: (projectId: number) => ipcRenderer.invoke('db:project:get', projectId),
    getAll: (options?: unknown) => ipcRenderer.invoke('db:project:getAll', options),
    update: (projectId: number, updates: unknown) =>
      ipcRenderer.invoke('db:project:update', projectId, updates),
    delete: (projectId: number) => ipcRenderer.invoke('db:project:delete', projectId)
  },
  ...
  analyses: {
    ...
    get: (analysisId: number) => ipcRenderer.invoke('db:analysis:get', analysisId),
    ...
    delete: (analysisId: number) => ipcRenderer.invoke('db:analysis:delete', analysisId)
  },

  // SEO Rules
  rules: {
    create: (ruleData: unknown) => ipcRenderer.invoke('db:rule:create', ruleData),
    get: (ruleId: number) => ipcRenderer.invoke('db:rule:get', ruleId),
    getAll: (options?: unknown) => ipcRenderer.invoke('db:rule:getAll', options),
  ...
  // Analysis Results
  results: {
    create: (resultData: unknown) => ipcRenderer.invoke('db:result:create', resultData),
    ...
    delete: (resultId: number) => ipcRenderer.invoke('db:result:delete', resultId),
  ...
  seo: {
    ...
    updateRecommendationStatus: (recId: number, status: string, notes?: string) =>
      ipcRenderer.invoke(
    ...
    saveRecommendations: (analysisId: number, enhancedRecommendations: unknown[]) =>
      ipcRenderer.invoke(
  ...
  keyword: {
    ...
    generateLongTail: (content: string, seedKeywords: string[], maxSuggestions?: number) =>
      ipcRenderer.invoke(
    ...
    generateLSI: (content: string, mainKeywords: string[], maxSuggestions?: number) =>
      ipcRenderer.invoke(
        'keyword:generateLSI',
        content,
        mainKeywords,
  ...
  readability: {
    ...
    analyzeLanguageGuidance: (content: string, options?: { language?: string }) =>
      ipcRenderer.invoke(
