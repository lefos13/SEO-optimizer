...
interface KeywordClusterResults {
...
const KeywordClusterer: React.FC = () => {
  const [keywords, setKeywords] = useState('');
  const [results, setResults] = useState<KeywordClusterResults | null>(null);
  ...
  const handleCluster = async () => {
    ...
    try {
      const result = (await window.electronAPI.keyword.cluster(
        keywordList,
        ''
      )) as unknown as KeywordClusterResults;
      setResults(result);
      scrollToResults();
    } catch (err) {
