...
interface SuggestionComponents {
  relevance: number;
  length: number;
  specificity: number;
}
...
interface ByIntent {
  ...
  transactional?: Suggestion[];
  navigational?: Suggestion[];
}

interface LongTailResults {
  totalPhrases: number;
...
const LongTailKeywordGenerator: React.FC = () => {
  const [content, setContent] = useState('');
  const [seedKeywords, setSeedKeywords] = useState('');
  const [maxSuggestions, setMaxSuggestions] = useState(20);
  const [results, setResults] = useState<LongTailResults | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [activeIntent, setActiveIntent] = useState<SearchIntent>('all');
  ...
  const getSuggestionsToShow = (): Suggestion[] => {
    if (!results) return [];
